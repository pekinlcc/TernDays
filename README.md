...
- 🧩 **桌面小组件**（Android / iOS）：今年 Top 3 城市及天数，打卡后自动刷新（每天至多两次），适配深色模式
...
- 城市边界附近仍可能判给邻市（离线库按最近点匹配，如燕郊可能判为北京）。深港/珠澳边界已在 v0.5 逐点修正并加交叉验证；遇到误判可直接在应用里手动更正，原始坐标始终保留，城市库升级后历史记录自动重解析
