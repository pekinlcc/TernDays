...
| 只有一次打卡（早或晚） | 该城市 +1 天 |
| **进行中的今天**只打了一次 | 该城市先 **+0.5 天**，另半天打上后补满 |
| 全天无记录 | 计入「无记录」，可在设置中手动补记（今天不算漏记） |

- 打卡时间按**手机当前本地时间**；跨时区旅行时监听系统时区变化自动重排闹钟。同一本地日期同一时段出现多条记录（向西飞）只取最早一条。
- 闹钟被系统推迟时在窗口内补打（早点 07:00–11:59，晚点 17:00–23:59），超窗记为无记录；
  定位失败会在窗口内自动重试一次（Android）。
- **今天**缺的那半天只要补捕窗口还没关，就先按半天计——人还在城里待着，天数不该先记满。
...
- 🧩 **桌面小组件**（Android / iOS，2×2）：今年 Top 3 城市及天数，三行等权重；三种外观可切换（素面 / 系统材质 / 品牌渐变）；打卡后与跨零点自动刷新，跟随深色模式
...
- 🔒 无账号、无服务器、零联网（仅换手机迁移页面临时使用局域网点对点传输）；原始经纬度仅存本机，删除应用即清除
  - 打卡数据（含原始坐标）**不进系统云备份**（v0.11.3 起）：Android 12+ 的云备份排除数据库与偏好，Android 11 及以下整体不备份；
    iOS 数据目录标记为不进 iCloud / 电脑备份。Android 本地换机（设备间直连）仍可带走；跨平台或 iOS 换机请用应用内「迁移到新手机」
