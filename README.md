...
- 📅 首页年度「城市 × 天数」列表；城市详情页有月历视图（全天 / 半天 / 进行中 / 其他城市 / 无记录）与打卡明细
- 🌏 **按国家 / 地区汇总**（中国大陆、香港、澳门、台湾分开统计）；可设**天数提醒**（如「中国大陆 183 天」，
  按自然年或最近 180 天计），接近或达到时本地提醒
- 🧭 **行程时间线**：同城连续的日子合成一段，首页显示「当前：上海 · 连续第 12 天」
- 📤 导出 **Excel (.xlsx)** 或 **CSV**：按年、最近 180 天或自定义区间；城市汇总 + 每日明细（含时区列）+ 行程段，本机生成走系统分享，不联网
...
- ✍️ 补记可选任意过去的日期，也能**一次补一段日子**（首日下午到 / 末日中午走可只记半天）；**任何一天都可手动更正城市**
  （首页「纠正」/ 城市详情月历点任意一天），手动更正优先于自动判定，写完可**一键撤销**
- 👀 打卡状态看得见：今日卡片显示「最近一次尝试」与「下一次」；最近几天漏记时首页给出诊断；失败通知带「立即打卡」按钮
- 💾 **加密文件备份 / 恢复**：口令加密（PBKDF2 + AES-GCM）保存到手机或网盘，恢复时本机已有记录优先；全程不联网
- ⏸️ 可暂停自动打卡（换机后停掉旧手机）；可清除本机全部数据
